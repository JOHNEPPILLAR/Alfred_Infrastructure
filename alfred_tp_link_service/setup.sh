...
echo "Setup/run alfred TP Link service docker container"
echo "-------------------------------------------------"
echo ""
...
case $SETUP_VAULT in
    Yes )   echo "Setup policies..."
            vault policy write -address=$VAULT_URL alfred_tp_link_service policy.hcl
            vault write -address=$VAULT_URL auth/approle/role/alfred_tp_link_service_role token_ttl=1m token_max_ttl=2m token_policies=alfred_tp_link_service  
            ;;
...
echo "Creating certs..."
mkcert alfred_tp_link_service

echo "Storing certs..."
vault write -address=$VAULT_URL secret/alfred_tp_link_service/ssl_key data=@alfred_tp_link_service-key.pem
vault write -address=$VAULT_URL secret/alfred_tp_link_service/ssl_cert data=@alfred_tp_link_service.pem
echo "Tidying up certs..."
rm *.pem

echo "Creating access token..."
VAULES=$(vault read -address=$VAULT_URL -format=json auth/approle/role/alfred_tp_link_service_role/role-id)
APP_ROLE_ID=$(echo $VAULES | jq .data.role_id)
export APP_ROLE_ID=${APP_ROLE_ID:1:${#APP_ROLE_ID}-2}
VAULES=$(vault write -f --format=json -address=$VAULT_URL auth/approle/role/alfred_tp_link_service_role/secret-id)
