...
echo "Setup/run alfred nest data collector service docker container"
echo "-------------------------------------------------------------"
echo ""
...
echo "Creating certs..."
mkcert alfred_nest_data_collector_service

echo "Storing certs..."
vault write -address=$VAULT_URL secret/alfred_nest_data_collector_service/ssl_key data=@alfred_nest_data_collector_service-key.pem
vault write -address=$VAULT_URL secret/alfred_nest_data_collector_service/ssl_cert data=@alfred_nest_data_collector_service.pem
echo "Tidying up certs..."
rm *.pem

echo "Creating access token..."
VAULES=$(vault read -address=$VAULT_URL -format=json auth/approle/role/alfred_nest_data_collector_service_role/role-id)
APP_ROLE_ID=$(echo $VAULES | jq .data.role_id)
export APP_ROLE_ID=${APP_ROLE_ID:1:${#APP_ROLE_ID}-2}
VAULES=$(vault write -f --format=json -address=$VAULT_URL auth/approle/role/alfred_nest_data_collector_service_role/secret-id)
